...
class AIService:
    ...
    def _build_prompt(self, landmarks_count: int, metrics: Dict) -> str:
        ...
        if adams_block:
            analysis_request = (
                "请严格按照以下格式输出，使用【】作为段落标记：\n"
                "【健康等级】综合站立位和弯腰位数据给出最终等级（正常/轻度/中度/重度）\n"
                "【判断依据】逐条列出各项指标数值及对应阈值，说明为何判定为该等级；同时解读弯腰位躯干对称性（肩骨盆扭转和侧移是否提示姿势性或结构性问题）\n"
                "【对应建议】分点列出复查频率、运动方案、生活调整建议、是否需要就医\n"
                "【重要提醒】一句个性化提醒\n"
            )
        else:
            analysis_request = (
                "请严格按照以下格式输出，使用【】作为段落标记：\n"
                "【健康等级】根据测量数据给出等级（正常/轻度/中度/重度）\n"
                "【判断依据】逐条列出各项指标数值及对应阈值，说明为何判定为该等级\n"
                "【对应建议】分点列出复查频率、运动方案、生活调整建议、是否需要就医\n"
                "【重要提醒】一句个性化提醒\n"
            )
        ...
        return (
            ...
            f"{adams_block}\n"
            f"{analysis_request}\n"
            "务必使用【】标记每段开头，段落内用换行分点，语言简洁，适合家长阅读。"
        )
